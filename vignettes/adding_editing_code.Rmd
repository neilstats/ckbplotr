...
The `make_shape_plot()` and `forest_plot()` functions generate code to produce plots. Beyond the function arguments, there are some further methods to change the generated code to produce customised plots.

## Adding layers
The plots returned by `make_shape_plot()` and `forest_plot()` are ggplot objects, so you can use `+` to add more layers or adjust the theme.
...
The addcode argument in `make_shape_plot()` and `forest_plot()` can be used to add additional code to the plot code created by the functions.
...
The addaes and addarg arguments in `make_shape_plot()` and `forest_plot()` can be used to add additional aesthetics and arguments code to the ggplot layers created by the functions. These arguments must be named lists of character strings, and the names of elements defines where the aesthetics/arguments code is added.
...
This can be used to 'overwrite' some of the code that would otherwise be produced by `make_shape_plot()` and `forest_plot()`.
...
The `make_shape_plot()` and `forest_plot()` functions return plot code in a character vector. You could change elements of the vector to customise the code, then parse and evaluate it using `eval(parse(text = plotcode))`.
