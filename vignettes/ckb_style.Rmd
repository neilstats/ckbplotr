...
2. extends the plotting area and caps the axis lines (so that you can have a custom sized gap between the plotting area and the axes)
...
Use `+ ckb_style()` to apply a CKB theme, cap axis lines and fix the aspect ratio.
...
The `ink` and `paper` arguments of `theme_ckb()` and `ckb_style()` can be used to change the colours of the plot.
```{r colour}
plot + ckb_style(xlims = c(0, 8),
                 ylims = c(10, 50),
                 ink = "darkred",
