...
## Using groups
The `col.group` argument can be supplied to plot results for different groups (using shades of grey for the fill colour). Use the `legend.position` to set the legend position.
...
If you do not want to include the text above points in the plot, then the addaes argument can be used to set `label` to `NA`. This will cause a warning from ggplot2 about rows containing missing values.
```{r remove-estimates-text, fig.height = 4, fig.width = 4, out.width = "50%"}
shape_plot(ckbplotr_shape_data[ckbplotr_shape_data$is_female == 0,],
           col.x        = "rf",
           col.estimate = "est",
           col.stderr   = "se",
...
See ["Changing generated code"](adding_editing_code.Rmd) for more on the addaes argument.
