...
When making a forest plot, if a confidence interval line is narrower than the point representing the estimate we may wish to plot the line differently. In this package, this is possible by using the `panel.width` argument in the `forest_plot()` function, then using the `fix_panel()` function.
...
## Changing the colour
To plot narrow confidence widths a different colour, set the `panel.width` argument of `forest_plot()` equal to the width each  panel will be in the final drawn output. The `cicolour` argument should also be a character vector - the last element will be used for narrow confidence intervals.
