...
(one data frame for each forest plot panel). Here we generate some example results.
...
To use row labels, create a data frame of labels and specify arguments `row.labels` and
`rows`.
...
giving labels from the heading1 column of the row.labels data frame.
...
will be in the order that they are in the row.labels data frame.)
