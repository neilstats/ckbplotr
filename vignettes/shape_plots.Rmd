...
## Basic usage
Supply a data frame of estimates and standard errors to the `shape_plot()` function and specify the name of the column that contains x-axis values using `col.x`. Use `col.estimate` and `col.stderr` to specify the names of columns containing estimates and standard errors (some common names are found automatically).
```{r}
...
Set axis limits using `xlims` and `ylims`, and axis titles using `xlab` and `ylab`.
```{r}
shape_plot(my_results,
           col.x        = "risk_factor",
           xlims        = c(15, 30),
...
Setting `ylims` will also make the function add code to show arrows where confidence interval lines exceed the limits.
```{r}
shape_plot(my_results,
           col.x        = "risk_factor",
           xlims        = c(15, 30),
...
The risk factor can be a factor. You may need to add position arguments so that points, intervals and text do not overlap.
```{r}
