...
The `forest_plot()` function creates a forest plot using the [ggplot2](https://ggplot2.tidyverse.org/) graphics package. The function returns both a plot and the ggplot2 code used to create the plot. In RStudio, the code used to create the plot will be shown in the Viewer pane.
...
```
Use `col.est` and `col.stderr` to set the columns that contain estimates and standard errors. By default, the function will look for columns with names estimate/est/beta/loghr and stderr/std.error/std.err/se. If you want to supply confidence interval limits, set `col.lci` and `col.uci`.
```{r, include = FALSE}
...
To add row labels, create a data frame with your row labels and one column that matches the `col.key` column.
```{r}
...
Use `col.left` and `col.right` to add columns of text either side of each panel. Use `left.heading` and `right.heading` to customise the column headings.
...
```
Use `left.hjust` and `right.hjust` to set the horizontal justification of the columns (0 = left, 0.5 = center, 1 = right).
...
If `xlim`, `xticks` and `panels` are lists of the same length, then `forest_plot()` will do this automatically. The function will return a list containing "figure" (a graphic object of the whole figure) and "plots" (the result of calls to `forest_plot()` for each panel).
...
The automatic positioning of columns and spacing of panels does not take into account this additional text, so you may need to use the `right.space` and `right.pos` arguments for a satisfactory layout.
