...
*This package is under development. If you find an error or bug or have a suggestion for improvement please create an issue on GitHub or contact the author at [neil.wright@ndph.ox.ac.uk](mailto:neil.wright@ndph.ox.ac.uk), [\@NeilStats](https://twitter.com/NeilStats), or [\@NeilStats\@fediscience.org](https://fediscience.org/@neilstats).*

It can be used to: create plots of estimates and CIs against risk factor levels...
```{r example-shape-plot, echo = FALSE, warning = FALSE, message = FALSE, out.width = "50%"}
...
forestplot <- forest_plot(row.labels        = ckbplotr_row_labels,
                          ...
                          panels            = list("Analysis A" = resultsA,
                                                   "Analysis B" = resultsB),
                          exponentiate      = TRUE,
                          col.key           = "variable",
                          ci.delim          = " - ",
...
arrow <- grid::textGrob("\u2192")
ckbplot <- plot + ckb_style(xlims = c(0, 8), ylims = c(10, 50))
...
Read `vignette("ckbplotr")` to see how to use `shape_plot()`, `forest_plot()`, and `ckb_style()`.
