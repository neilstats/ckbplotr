...
forest.columns.right <- function(col.right.all,
  ...
  x <- unlist(purrr::pmap(
    list(column     = col.right.all,
         ...
         xpos       = round(axis_scale_inverse_fn(axis_scale_fn(xto) + (axis_scale_fn(xto) - axis_scale_fn(xfrom))), 6),
...
forest.columns.left <- function(col.left,
  ...
  x <- unlist(purrr::pmap(
    list(column     = col.left,
         ...
         bold       = if (is.null(col.bold)) FALSE else col.bold,
         parse      = FALSE,
         xpos       = round(axis_scale_inverse_fn(axis_scale_fn(xfrom) - (axis_scale_fn(xto) - axis_scale_fn(xfrom))), 6),
